...
1. **Lark Base** (`lark`): easiest from the running dashboard, Settings → Lark Base. Paste a link to the Base (`https://<domain>/base/<baseToken>?table=…&view=…`, copied from Lark’s address bar), then choose the table and view for Tasks, Issues and Merchant Feedback; the Tasks view is normally your own. The dashboard writes all of it into the config for you. By hand, the same values live in `lark.domain`, `lark.baseToken` and `lark.tables.tasks|issues|feedback`. The `fields` blocks map the dashboard's names to your column headers; list your columns with `lark-cli base +field-list --base-token <baseToken> --table-id <tableId> --format json --as user` and change any header that differs. Optional columns (reporter, PIC, attachments, form link, feedback status grouping) are documented in the README. The Tasks view can be switched later from the dashboard, Settings â Tasks view, without touching the file.
