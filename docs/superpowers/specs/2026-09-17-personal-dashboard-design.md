...
**Pull Requests**: three sub-lists toggled by tabs inside the panel: "Review requested", "Mine" and, added 2026-09-18, "All", which lists every open PR in the repositories configured as `github.repos` (chosen on the Settings page, up to 50), most recently updated first. The GraphQL request adds one alias per repository (`rN: repository(owner, name) { pullRequests(states: OPEN, first: 50, orderBy: UPDATED_AT DESC) }`); an alias that fails (renamed, deleted or inaccessible repository) is ignored rather than failing the poll, even though `gh` then exits non-zero. The All list produces no events and no attention chips. Row: repo short name, `#number`, title, author avatar + login, age, CI dot (green/red/grey), and one status badge: Draft while the PR is a draft (a draft has no meaningful review decision), otherwise the review decision (Approved / Changes requested / Review required). Click opens the PR on GitHub in a new tab.
...
**Pull requests** (added 2026-09-18): `GET /api/github/repos` lists the signed-in account's repositories via GraphQL (`viewer.repositories`, owner/collaborator/organization affiliations, up to 300, newest push first, with open PR counts) together with the current selection; `PUT /api/github/repos { repos }` validates `owner/name` strings (max 50), rewrites `github.repos` in the config file atomically through `server/configEdit.ts`, and rebuilds the GitHub source so the next poll runs at once. The card keeps selected repositories that are no longer visible in the list so they can be unticked.
